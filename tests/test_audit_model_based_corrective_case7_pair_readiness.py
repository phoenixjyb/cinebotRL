...
SPEC = importlib.util.spec_from_file_location("case7_pair_readiness", SCRIPT)
...
def test_current_case7_is_evidence_ready_but_not_profile_ready() -> None:
    result = _audit()
    ...
    assert "case7_role" in result["selection_checks"]
    assert "case7_checks" in result["selection_checks"]
...
def test_case7_selected_hashes_and_dynamic_gate_are_exact() -> None:
    result = _audit()
    row = next(
        row
