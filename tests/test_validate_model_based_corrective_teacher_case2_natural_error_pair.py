...
def test_validator_rejects_authorization_or_alternate_contract(
    tmp_path, monkeypatch
) -> None:
    repo, contract = _fixture_repo(tmp_path, monkeypatch)
    token = tmp_path / "token"
    token.write_text("not accepted\n", encoding="utf-8")
    authorized = MODULE.validate(
        contract,
        repo,
        namespace=MODULE.NAMESPACE,
        authorization_file=token,
    )
    assert authorized["checks"]["authorization_file_absent"] is False
    ...
    assert authorized["passed"] is changed["passed"] is False
...
def test_wrapper_rejects_supplied_token_and_environment_override(
    tmp_path,
) -> None:
    token = tmp_path / "token"
    ...
    assert supplied.returncode == 4
    assert json.loads(supplied.stderr)["reason"] == (
