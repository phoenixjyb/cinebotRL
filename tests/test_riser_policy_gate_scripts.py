...
def test_masked_previous_action_bc_runner_is_diagnosis_bound_and_guarded() -> None:
    source = _read("run_riser_masked_previous_action_bc.sh")
    assert 'DIAGNOSIS_COMMIT="6f5940c' in source
    assert 'DIAGNOSIS_SHA256="9c81451e' in source
    assert "autoregressive_previous_action_exposure_bias" in source
    assert "--mask-previous-action-observations" in source
    assert '"masked_observation_indices": [23, 24, 25]' in source
    assert "state_shared_lookahead_fusion_previous_action_masked_v1" in source
    assert "RISER_MASKED_BC_AUTHORIZATION_FILE" in source
...
def test_camera_lever_arm_gate_is_ordered_bounded_and_training_closed() -> None:
    ...
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE32_V15_EXPLICIT_PREVIEW0175_V1" in source
    assert "20260719_gate_c_smoothed_case32_v15_explicit_preview0175_v1_exclusive" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE32_V15_EXPLICIT_PREVIEW0175_V2" in source
    assert "20260719_gate_c_smoothed_case32_v15_explicit_preview0175_v2_exclusive" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE32_V15_EXPLICIT_PREVIEW0175_V3" in source
    assert "20260719_gate_c_smoothed_case32_v15_explicit_preview0175_v3_exclusive" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE32_V15_EXPLICIT_PREVIEW0175_V4" in source
    assert "20260720_gate_c_smoothed_case32_v15_explicit_preview0175_v4_exclusive" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE33_V15_CAMERA_LEVER_ARM_V1" in source
    assert "20260720_gate_c_smoothed_case33_v15_camera_lever_arm_v1_exclusive" in source
    assert "052b828587efef44e8f17bc6c8a4d73dcbfc2d35466ae02f5dd1a60f64af8d00" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE34_V15_CAMERA_LEVER_ARM_V1" in source
    assert "20260720_gate_c_smoothed_case34_v15_camera_lever_arm_v1_exclusive" in source
    assert "e2b170f649f9e90542bfaa463c74fa802c0247273d7fad8c26f24922c212b9d4" in source
    ...
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE37_V16_CAMERA_LEVER_ARM_V1" in source
    assert "20260720_gate_c_smoothed_case37_v16_camera_lever_arm_v1_exclusive" in source
    assert "3bc3119fa210f1fd190d7fba11b9571caa74dc1bc4de02fb98296ecc9e8d2c1e" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE41_V16_CAMERA_LEVER_ARM_V1" in source
    assert "20260720_gate_c_smoothed_case41_v16_camera_lever_arm_v1_exclusive" in source
    assert "cf3c1f35fbf20377c23dbc7ff3d24fbca8cdc9ef833cf1eff925d585295a4679" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE42_V17_STATIC_MARGIN_PREVIEW055_G125_V1" in source
    assert "20260720_gate_c_smoothed_case42_v17_static_margin_preview055_g125_v1_exclusive" in source
    assert "57f6eab7453d0d1d3f2c244b7d429bb5ac1fa95184f63b4212770dc9fefb1a51" in source
    assert "b2057b0a23c9b5172f09a5373a020b18583cf118d55886624332f1d4e861a298" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE42_V18_LOCALIZED_REVERSAL_RETIME_V1" in source
    assert "20260720_gate_c_smoothed_case42_v18_localized_reversal_retime_v1_exclusive" in source
    assert "b296a32f64a3e1f22b3a2fb51db2dd426baa2e1374d36cdf0eaaf5a5cdecd5a1" in source
    assert "df694d8e6702ac0712ff7e1ce597c79ac30e3fc08f072caf63245fe8740e6669" in source
    assert "4d5db5a5a5c9977c53f25a5d0cc744f94962071e" in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE42_V16_BASELINE_ROLLBACK_V1" in source
    assert "20260720_gate_c_smoothed_case42_v16_baseline_rollback_v1_exclusive" in source
    assert "8351514a361d3be4e5fbf57f2dbb019a7d8d2f5b86e89cea2553a1cfda3c64a1" in source
    ...
    assert "--use-root-velocity-outer-feedback" in source
    assert 'balance_controller "$METRICS"' in source
    assert 'result.get("outer_velocity_feedback_source") == "root_link_vx"' in source
    assert 'velocity_feedback.get("policy_rate_sample_count")' in source
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE42_V20_ZERO_PROGRESS_HOLD_V1" in source
    assert "20260720_gate_c_smoothed_case42_v20_zero_progress_hold_v1_exclusive" in source
    assert 'REQUIRE_ZERO_PROGRESS_HOLD=1' in source
    assert 'TRACKING_MINIMUM_PROGRESS_SCALE="0.0"' in source
