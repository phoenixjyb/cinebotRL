...
VALIDATOR = (
    ...
    "validate_model_based_corrective_teacher_case32_validation_capture.py"
)
BUILDER = (
    ROOT
    / "scripts/two_wheel_balance/"
    "build_model_based_corrective_teacher_case32_validation_capture_contract.py"
)
WRAPPER = (
    ROOT
    / "scripts/two_wheel_balance/"
    "run_model_based_corrective_teacher_case32_validation_capture.sh"
)
FINALIZER = (
    ROOT
    / "scripts/two_wheel_balance/"
    "summarize_model_based_corrective_teacher_case32_validation_capture.py"
)
...
MODULE = _module(VALIDATOR, "case32_validation_capture_validator")
FINALIZER_MODULE = _module(FINALIZER, "case32_validation_capture_finalizer")
...
def test_contract_is_validation_only_tokenless_and_hash_bound() -> None:
    ...
    assert contract["validation_cases_opened"] == [32]
...
def test_wrapper_is_one_validation_capture_without_training() -> None:
    ...
    assert "case32_validation_capture_v1_coexistence" in source
    assert "case32_validation_natural_error_profile_v1.json" in source
    assert "--deterministic-wrench-profile" not in source
    assert (
        'AUTHORIZATION_SHA256="${'
        'RISER_CORRECTIVE_CASE32_VALIDATION_CAPTURE_AUTHORIZATION_SHA256:-}"'
    ) in source
...
def test_authorization_hash_must_be_external_and_exact(tmp_path: Path) -> None:
    ...
    token.write_text("one case-32 validation capture\n", encoding="utf-8")
    token.chmod(0o600)
...
def test_finalizer_uses_case32_validation_archive(monkeypatch, tmp_path) -> None:
    observed = {}
    ...
    monkeypatch.setattr(FINALIZER_MODULE, "summarize_shared", fake_summary)
    ...
    assert observed["expected_case"] == 32
    assert observed["expected_namespace"] == MODULE.NAMESPACE
    assert observed["capture_name"] == (
        "case_0032_corrective_teacher_capture_v2.npz"
    )
    assert observed["plan_identity_name"] == "case32_plan"
    assert observed["expected_perturbation_rows"] == 0
