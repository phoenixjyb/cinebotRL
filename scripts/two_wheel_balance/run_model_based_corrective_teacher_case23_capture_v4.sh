...
readonly NAMESPACE="20260723_model_based_corrective_teacher_case23_capture_v4_exclusive"
readonly CONTRACT="$ROOT/scripts/two_wheel_balance/model_based_corrective_teacher_case23_capture_contract_v4.json"
readonly VALIDATOR="$ROOT/scripts/two_wheel_balance/validate_model_based_corrective_teacher_case23_capture_v4.py"
...
readonly FINALIZER="$WIN_ROOT\scripts\two_wheel_balance\summarize_model_based_corrective_teacher_case23_capture_v4.py"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CORRECTIVE_CASE23_CAPTURE_V4_NAMESPACE \
  RISER_CORRECTIVE_CASE23_CAPTURE_V4_CONTRACT \
  RISER_CORRECTIVE_CASE23_CAPTURE_V4_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE23_CAPTURE_V4_AUTHORIZATION_FILE:-}"
AUTHORIZATION_SHA256="${RISER_CORRECTIVE_CASE23_CAPTURE_V4_AUTHORIZATION_SHA256:-}"
