...
actions = np.array(actions_list)  # [num_samples, 9]

print(f"\nAction Statistics (from {num_samples} random observations):")
print(f"Action space: 9 dimensions")
print(f"  - Dimensions 0-5: Arm joint targets")
print(f"  - Dimension 6: Base linear velocity (v_x)")
print(f"  - Dimension 7: Base lateral velocity (v_y)")
print(f"  - Dimension 8: Base angular velocity (omega_z)")
print()
...
base_vy_mean = abs(actions[:, 7].mean())
base_wz_mean = abs(actions[:, 8].mean())
base_vx_std = actions[:, 6].std()
base_vy_std = actions[:, 7].std()
base_wz_std = actions[:, 8].std()

if base_vx_mean < 0.01 and base_vy_mean < 0.01 and base_wz_mean < 0.01:
    print("⚠️  WARNING: Base velocity commands are VERY SMALL!")
    print(f"   Base V_X mean: {base_vx_mean:.6f}")
    print(f"   Base V_Y mean: {base_vy_mean:.6f}")
    print(f"   Base OMEGA_Z mean: {base_wz_mean:.6f}")
    print("   → Policy may not have learned to move the base")
elif base_vx_std > 1.0 or base_vy_std > 1.0 or base_wz_std > 1.0:
    print("⚠️  WARNING: Base commands have VERY HIGH variance!")
    print(f"   Base V_X std: {base_vx_std:.4f}")
    print(f"   Base V_Y std: {base_vy_std:.4f}")
