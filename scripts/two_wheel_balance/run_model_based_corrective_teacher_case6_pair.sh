...
readonly CONTRACT="$SCRIPT_DIR/model_based_corrective_teacher_case6_pair_contract_v2.json"
readonly VALIDATOR="$SCRIPT_DIR/validate_model_based_corrective_teacher_case6_pair.py"
readonly NAMESPACE="20260724_model_based_corrective_teacher_case6_pair_v2_exclusive"
...
readonly FINALIZER="$WIN_ROOT\\scripts\\two_wheel_balance\\summarize_model_based_corrective_teacher_case6_pair.py"
# This one-use authorization opens only the paired canary namespace.
readonly AUTHORIZATION_SHA256="1b765e84d41cbbffc7119575fefd4e7fd07da2e169ad4c4502720cb80a60f970"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  ...
  RISER_CORRECTIVE_CASE6_PERTURBATION RISER_CORRECTIVE_CASE6_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE6_AUTHORIZATION_FILE:-}"
...
timeout --signal=TERM --kill-after=30s 600 \
  ...
  --output "$OUTPUT_WIN\\baseline\\case_0006.json" \
...
if [[ "$BASELINE_STATUS" == 0 && "$BASELINE_GPU_RELEASE_PASSED" == 1 ]] \
  && python3 - "$OUTPUT/baseline/case_0006.json" <<'PY'
import json
import sys
...
then
  ...
  timeout --signal=TERM --kill-after=30s 600 \
    ...
    --output "$OUTPUT_WIN\\candidate\\case_0006.json" \
