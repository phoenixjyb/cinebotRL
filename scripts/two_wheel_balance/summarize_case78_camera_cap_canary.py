#!/usr/bin/env python3
"""Seal one heartbeat-enabled case-78 camera-correction-cap canary."""
...
EXPECTED_CORRECTION_CAP_M = 0.10
...
def summarize(
    ...
    *,
    ...
    heartbeat_wall_s = max(0.0, heartbeat_emitted_s - wall_started_epoch_s)
    completed_steps = int(heartbeat.get("completed_steps", 0))
    observed_step_rate_hz = (
        completed_steps / heartbeat_wall_s if heartbeat_wall_s > 0.0 else None
    )
    admission_checks = {
    ...
    heartbeat_checks = {
        ...
        "capture_disabled": heartbeat.get("capture_outputs_enabled") is False,
    ...
    gate_contract_checks = {
        ...
        == EXECUTION_DURATION_S,
        "camera_lever_arm_enabled": gate.get(
            "camera_lever_arm_compensation_enabled"
        )
        is True
        and result.get("camera_lever_arm_compensation_enabled") is True,
        "camera_lever_arm_gain_exact": gate.get(
            "camera_lever_arm_compensation_gain"
        )
        == result.get("camera_lever_arm_compensation_gain")
        == 1.0,
        "camera_correction_cap_exact": gate.get(
            "maximum_camera_lever_arm_correction_m"
        )
        == result.get("maximum_camera_lever_arm_correction_m")
        == EXPECTED_CORRECTION_CAP_M,
        "camera_lever_arm_telemetry_observed": result.get(
            "camera_lever_arm_telemetry_observed"
        )
        is True
        and result.get("camera_lever_arm_telemetry_sample_count")
        == result.get("completed_steps"),
        "camera_correction_respects_cap": float(
            result.get("camera_lever_arm_correction_max_m", float("inf"))
        )
        <= EXPECTED_CORRECTION_CAP_M + 1e-9,
        "camera_recovery_disabled": gate.get(
    ...
    forbidden_outputs = [
        ...
        root / "raw_teacher",
        root / "policy_traces",
        root / "shadow_teacher_traces",
    ]
    no_data_checks = {
        "runtime_no_dataset": not result
        or result.get("executed_residual_dataset") is None,
        "runtime_no_raw_teacher": not result
        or result.get("executed_raw_teacher_capture") is None,
        "runtime_no_policy_trace": not result
        or result.get("executed_policy_trace") is None,
        "runtime_no_shadow_trace": not result
        or result.get("executed_shadow_teacher_trace") is None,
        "top_level_no_capture": not gate
        or (
            gate.get("raw_teacher_capture_started") is False
        ...
        "top_level_training_closed": not gate
        or (
            gate.get("training_started") is False
            and gate.get("dagger_authorized") is False
            and gate.get("ppo_authorized") is False
        ),
        "filesystem_no_data_outputs": not any(
            path.exists() for path in forbidden_outputs
        ),
    }
    heartbeat_passed = all(heartbeat_checks.values())
    timeout_evidence_preserved = playback_exit_code != 124 or heartbeat_passed
    ...
    return {
        "schema": "cinebotrl_two_wheel_riser_case78_camera_cap_final_v1",
        ...
        "heartbeat_age_at_finish_s": max(
            0.0, wall_finished_epoch_s - heartbeat_emitted_s
        ),
        "heartbeat_observed_policy_step_rate_hz": observed_step_rate_hz,
        ...
        "dataset_created": not all(no_data_checks.values()),
