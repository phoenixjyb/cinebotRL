...
readonly NAMESPACE="20260724_model_based_corrective_teacher_case6_capture_v1_exclusive"
readonly CONTRACT="$ROOT/scripts/two_wheel_balance/model_based_corrective_teacher_case6_capture_contract_v1.json"
readonly VALIDATOR="$ROOT/scripts/two_wheel_balance/validate_model_based_corrective_teacher_case6_capture.py"
...
readonly PLAN_DIR="$WIN_ROOT\docs\03_training\two_wheel_balance\evidence_20260724_case6_pair_readiness_cpu_v1\source"
readonly WRENCH_PROFILE="$WIN_ROOT\scripts\two_wheel_balance\model_based_corrective_teacher_case6_wrench_profile_v1.json"
readonly CORRECTIVE_PROFILE="$WIN_ROOT\scripts\two_wheel_balance\model_based_corrective_teacher_case6_profile_v1.json"
...
readonly FINALIZER="$WIN_ROOT\scripts\two_wheel_balance\summarize_model_based_corrective_teacher_case6_capture.py"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CORRECTIVE_CASE6_CAPTURE_NAMESPACE \
  RISER_CORRECTIVE_CASE6_CAPTURE_CONTRACT \
  RISER_CORRECTIVE_CASE6_CAPTURE_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE6_CAPTURE_AUTHORIZATION_FILE:-}"
AUTHORIZATION_SHA256="${RISER_CORRECTIVE_CASE6_CAPTURE_AUTHORIZATION_SHA256:-}"
