...
VALIDATOR = (
    SCRIPT_DIR
    / "validate_model_based_corrective_teacher_case32_validation_natural_error_pair.py"
)
BUILDER = (
    SCRIPT_DIR
    / "build_model_based_corrective_teacher_case32_validation_natural_error_pair_contract.py"
)
WRAPPER = (
    SCRIPT_DIR
    / "run_model_based_corrective_teacher_case32_validation_natural_error_pair.sh"
)
ADAPTER = (
    SCRIPT_DIR / "smoke_riser_case32_validation_natural_error_pair.py"
)
FINALIZER = (
    SCRIPT_DIR
    / "summarize_model_based_corrective_teacher_case32_validation_natural_error_pair.py"
)
...
VALIDATOR_MODULE = _module(VALIDATOR, "case32_validation_pair_validator")
ADAPTER_MODULE = _module(ADAPTER, "case32_validation_projection_adapter")
FINALIZER_MODULE = _module(FINALIZER, "case32_validation_pair_finalizer")
...
def test_adapter_injection_is_case32_validation_only(tmp_path: Path) -> None:
    output = tmp_path / "case32.json"
    output.write_text(
        json.dumps({"results": [{"case": 32}]}), encoding="utf-8"
    )
    ...
    adapter = payload["case32_validation_projection_telemetry_adapter"]
    ...
    with pytest.raises(ValueError, match="case-32"):
        ADAPTER_MODULE.inject_projection_telemetry(output, telemetry)
...
def test_contract_is_hash_bound_validation_only_and_tokenless() -> None:
    ...
    assert contract["case"] == 32
    assert contract["split"] == "validation"
    assert contract["selected_validation_cases"] == [8, 32]
...
def test_wrapper_rejects_conflicting_environment_before_python() -> None:
    env = dict(os.environ)
    env["RISER_CORRECTIVE_CASE32_VALIDATION_PERTURBATION"] = "/tmp/forged"
    result = _run(
...
def test_wrapper_has_no_wrench_capture_or_dataset_route() -> None:
    ...
    token_hash = source.index(
        "RISER_CORRECTIVE_CASE32_VALIDATION_AUTHORIZATION_SHA256"
    )
    ...
    assert "case32_validation_natural_error_pair_v1_coexistence" in source
    assert "--cases 32" in source
    assert "case_0032.json" in source
    assert "case32_validation_natural_error_profile_v1.json" in source
...
def test_validator_recognizes_valid_out_of_band_authorization(
    ...
    token.write_text(
        "one bounded case-32 validation natural-error pair\n",
        encoding="utf-8",
    )
    token.chmod(0o600)
...
def _gate(
    *,
    ...
    source_duration_s: float = 21.648708,
) -> dict[str, object]:
    projection = {
        "schema": (
...
def _fixture(tmp_path: Path, *, candidate_p95: float = 0.070):
    ...
    contract = {
        ...
        "case": 32,
        "split": "validation",
        "controller_arguments": {"reset_seed": 20260748},
        "identities": {
            "case32_plan": {"sha256": "a" * 64},
