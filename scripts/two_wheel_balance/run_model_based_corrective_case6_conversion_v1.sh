...
readonly NAMESPACE="20260724_model_based_corrective_case6_conversion_v1_cpu"
readonly CONTRACT="$ROOT/scripts/two_wheel_balance/model_based_corrective_case6_conversion_execution_contract_v1.json"
readonly CONTRACT_WIN="$WIN_ROOT\scripts\two_wheel_balance\model_based_corrective_case6_conversion_execution_contract_v1.json"
readonly VALIDATOR_WIN="$WIN_ROOT\scripts\two_wheel_balance\validate_model_based_corrective_case6_conversion_execution.py"
readonly CONVERTER_WIN="$WIN_ROOT\scripts\two_wheel_balance\convert_model_based_corrective_capture.py"
readonly FINALIZER_WIN="$WIN_ROOT\scripts\two_wheel_balance\finalize_model_based_corrective_case6_conversion.py"
readonly SOURCE_CAPTURE_WIN="$WIN_ROOT\docs\03_training\two_wheel_balance\evidence_20260724_case6_corrective_capture_v1\capture\case_0006_corrective_teacher_capture_v2.npz"
readonly SOURCE_FINAL_WIN="$WIN_ROOT\docs\03_training\two_wheel_balance\evidence_20260724_case6_corrective_capture_v1\final_status.json"
readonly OUTPUT="$ROOT/artifacts/two_wheel_riser/$NAMESPACE"
readonly OUTPUT_WIN="${WIN_ROOT}\\artifacts\\two_wheel_riser\\${NAMESPACE}"
readonly DATASET_WIN="${OUTPUT_WIN}\\case_0006_model_based_corrective_case_dataset_v1.npz"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CASE6_CONVERSION_NAMESPACE \
  RISER_CASE6_CONVERSION_CONTRACT \
  RISER_CASE6_CONVERSION_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CASE6_CONVERSION_AUTHORIZATION_FILE:-}"
AUTHORIZATION_SHA256="${RISER_CASE6_CONVERSION_AUTHORIZATION_SHA256:-}"
...
mkdir -p "$TEMP_ROOT"
ADMISSION="$TEMP_ROOT/.case6_conversion_admission.$$.json"
ADMISSION_WIN="$WIN_ROOT\artifacts\two_wheel_riser\.case6_conversion_admission.$$.json"
