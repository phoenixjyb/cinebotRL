...
AUTHORIZATION="AUTHORIZED_RISER_SMOOTHED_GATE_C_TRANCHE2_24_19_28_70_26_HORIZON300_V1"
CASES="24,19,28,70,26"
STAMP="20260718_gate_c_smoothed_tranche2_24_19_28_70_26_horizon300_wzkp105_v1_exclusive"
...
CASE24_PLAN_SHA256="598493ee1ab6dd79223ef9f4a4591e6a4d447a978e87eb70d803a8eed6b4aee6"
CASE19_PLAN_SHA256="8cf8bde298c73d1809c3dc7c0dae249446d7554ba77275a490d32fc1a6004b37"
CASE28_PLAN_SHA256="e46de958090bea0a1f2a70b27d7ce9f5dbe62190407e121d74d6866fc9ecaee4"
CASE70_PLAN_SHA256="bbdb8ec625daf650f36cd1703999989f5cbd219300066417991e3babe34d0390"
CASE26_PLAN_SHA256="66d6f491da71521928a7b9012fd929cd7721fc677d2cf432d69cf224f470b415"
...
if [[ "${RISER_SMOOTHED_GATE_C_TRANCHE2_AUTHORIZATION:-}" != "$AUTHORIZATION" ]]; then
  printf 'tranche-2 Gate C authorization is absent or unknown\n' >&2
  exit 7
fi
...
RUNNER="$ROOT/scripts/two_wheel_balance/run_riser_smoothed_gate_c_tranche2.sh"
...
assert_gpu_free() {
  ...
  if [[ -n "$playback_owners" || -n "$compute_owners" ]]; then
    printf 'tranche-2 GPU is not free\n' >&2
...
assert_no_competing_cpu() {
  ...
  if [[ -n "$competing_cpu" ]]; then
    printf 'tranche-2 CPU/disk ownership is not exclusive\n%s\n' "$competing_cpu" >&2
    return 1
  fi
}
...
wait_for_gpu_release() {
  ...
  printf 'tranche-2 GPU did not release within 90 seconds\n' >&2
  assert_gpu_free
}
...
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0024_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE24_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0019_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE19_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0028_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE28_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0070_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE70_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0026_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE26_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$GAINS_WSL" | awk '{print $1}')" == "$GAINS_SHA256" ]] || exit 2
[[ "$(sha256sum "$ROBOT_USD" | awk '{print $1}')" == "$ROBOT_USD_SHA256" ]] || exit 2

"$PY" -X utf8 - \
  "$PORTFOLIO_WIN\case_0024_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0019_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0028_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0070_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0026_smoothed_riser_plan_v1.npz" <<'PY'
import json
import sys
...
for path, case in zip(sys.argv[1:], (24, 19, 28, 70, 26), strict=True):
    ...
    if not all(checks.values()):
        raise SystemExit(f"invalid tranche-2 case {case}: {checks}")
PY
...
python3 - "$TEMP_ADMISSION" "$COMMIT" "$STAMP" \
  ...
  case24_plan "$PORTFOLIO_WSL/case_0024_smoothed_riser_plan_v1.npz" \
  case19_plan "$PORTFOLIO_WSL/case_0019_smoothed_riser_plan_v1.npz" \
  case28_plan "$PORTFOLIO_WSL/case_0028_smoothed_riser_plan_v1.npz" \
  case70_plan "$PORTFOLIO_WSL/case_0070_smoothed_riser_plan_v1.npz" \
  case26_plan "$PORTFOLIO_WSL/case_0026_smoothed_riser_plan_v1.npz" \
...
for CASE in 24 19 28 70 26; do
  ...
  if [[ ! -s "$OUTPUT_WSL/gates/case_$(printf '%04d' "$CASE").json" ]] \
    ...
    python3 "$SUMMARIZER" --root "$OUTPUT_WSL" --git-commit "$COMMIT" \
      --cases "$CASES" --output "$OUTPUT_WSL/summary.json" >/dev/null
    printf 'tranche-2 Gate C stopped on case %s\n' "$CASE" >&2
    exit 4
  fi
...
ok = (
    summary.get("requested_cases") == [24, 19, 28, 70, 26]
    and summary.get("dynamically_passed_cases") == [24, 19, 28, 70, 26]
...
raise SystemExit(0 if ok else 6)
PY
printf 'tranche-2 Gate C closed: %s\n' "$OUTPUT_WSL"
