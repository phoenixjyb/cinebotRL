...
AUTHORIZATION="AUTHORIZED_RISER_SMOOTHED_GATE_C_TRANCHE3_25_66_68_67_7_HORIZON300_V1"
CASES="25,66,68,67,7"
STAMP="20260718_gate_c_smoothed_tranche3_25_66_68_67_7_horizon300_wzkp105_v1_exclusive"
...
CASE25_PLAN_SHA256="ac47ef0a8fef58fc8face3e8800b63283fe2d72da27321daeb44d59b8fe555c7"
...
CASE67_PLAN_SHA256="e7acb5b9ca748645d878d360f357feb82e89b968f92d86c2639f2b74e03950e0"
CASE7_PLAN_SHA256="421f9f74a9f56cb79b49611355d9520489bf0bbe7204212ba169b84591fa4cd0"
...
if [[ "${RISER_SMOOTHED_GATE_C_TRANCHE3_AUTHORIZATION:-}" != "$AUTHORIZATION" ]]; then
  printf 'tranche-3 Gate C authorization is absent or unknown\n' >&2
  exit 7
fi
...
RUNNER="$ROOT/scripts/two_wheel_balance/run_riser_smoothed_gate_c_tranche3.sh"
...
assert_gpu_free() {
  ...
  if [[ -n "$playback_owners" || -n "$compute_owners" ]]; then
    printf 'tranche-3 GPU is not free\n' >&2
...
assert_no_competing_cpu() {
  ...
  if [[ -n "$competing_cpu" ]]; then
    printf 'tranche-3 CPU/disk ownership is not exclusive\n%s\n' "$competing_cpu" >&2
    return 1
  fi
}
...
wait_for_gpu_release() {
  ...
  printf 'tranche-3 GPU did not release within 90 seconds\n' >&2
  assert_gpu_free
}
...
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0025_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE25_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0066_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE66_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0068_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE68_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0067_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE67_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$PORTFOLIO_WSL/case_0007_smoothed_riser_plan_v1.npz" | awk '{print $1}')" == "$CASE7_PLAN_SHA256" ]] || exit 2
[[ "$(sha256sum "$GAINS_WSL" | awk '{print $1}')" == "$GAINS_SHA256" ]] || exit 2
[[ "$(sha256sum "$ROBOT_USD" | awk '{print $1}')" == "$ROBOT_USD_SHA256" ]] || exit 2

"$PY" -X utf8 - \
  "$PORTFOLIO_WIN\case_0025_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0066_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0068_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0067_smoothed_riser_plan_v1.npz" \
  "$PORTFOLIO_WIN\case_0007_smoothed_riser_plan_v1.npz" <<'PY'
import json
import sys
...
expected = {
    25: (420, 8.097697, 13.159482674279998, "smoothed_preview_0.05m_g2.75"),
    66: (464, 9.096552, 13.56287393451732, "smoothed_preview_0.05m_g2.75"),
    68: (464, 9.096552, 13.562891285858488, "smoothed_preview_0.05m_g2.75"),
    67: (464, 9.096552, 13.562899771959323, "smoothed_preview_0.05m_g2.75"),
    7: (663, 12.940941, 13.582122465552235, "smoothed_preview_0.05m_g2.75"),
}
for path, case in zip(sys.argv[1:], (25, 66, 68, 67, 7), strict=True):
    ...
    if not all(checks.values()):
        raise SystemExit(f"invalid tranche-3 case {case}: {checks}")
PY
...
python3 - "$TEMP_ADMISSION" "$COMMIT" "$STAMP" \
  ...
  case25_plan "$PORTFOLIO_WSL/case_0025_smoothed_riser_plan_v1.npz" \
  case66_plan "$PORTFOLIO_WSL/case_0066_smoothed_riser_plan_v1.npz" \
  case68_plan "$PORTFOLIO_WSL/case_0068_smoothed_riser_plan_v1.npz" \
  case67_plan "$PORTFOLIO_WSL/case_0067_smoothed_riser_plan_v1.npz" \
  case7_plan "$PORTFOLIO_WSL/case_0007_smoothed_riser_plan_v1.npz" \
...
for CASE in 25 66 68 67 7; do
  ...
  if [[ ! -s "$OUTPUT_WSL/gates/case_$(printf '%04d' "$CASE").json" ]] \
    ...
    python3 "$SUMMARIZER" --root "$OUTPUT_WSL" --git-commit "$COMMIT" \
      --cases "$CASES" --output "$OUTPUT_WSL/summary.json" >/dev/null
    printf 'tranche-3 Gate C stopped on case %s\n' "$CASE" >&2
    exit 4
  fi
...
ok = (
    summary.get("requested_cases") == [25, 66, 68, 67, 7]
    and summary.get("dynamically_passed_cases") == [25, 66, 68, 67, 7]
...
raise SystemExit(0 if ok else 6)
PY
printf 'tranche-3 Gate C closed: %s\n' "$OUTPUT_WSL"
