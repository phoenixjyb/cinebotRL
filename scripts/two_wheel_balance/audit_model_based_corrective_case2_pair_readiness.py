#!/usr/bin/env python3
"""Audit case 2 before designing a case-specific paired canary."""
...
def _load_base_auditor():
    spec = importlib.util.spec_from_file_location(
        "case2_pair_readiness_shared_checks", BASE_AUDITOR
    )
...
def audit_readiness(
    ...
    """Apply the frozen paired-canary checks with case-2 identities."""

    result = _BASE.audit_readiness(selection_path, plan_path, gate_path)
    result["selection_checks"]["case2_role"] = result["selection_checks"].pop(
        "case6_role"
    )
    result["selection_checks"]["case2_checks"] = result[
        "selection_checks"
    ].pop("case6_checks")
    ...
    if not all(source_execution_clock_checks.values()):
        raise ValueError(
            "case-2 source/execution clock checks failed: "
