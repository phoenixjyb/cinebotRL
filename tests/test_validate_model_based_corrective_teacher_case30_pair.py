...
VALIDATOR = (
    ROOT
    / "scripts/two_wheel_balance/validate_model_based_corrective_teacher_case30_pair.py"
)
WRAPPER = (
    ROOT
    / "scripts/two_wheel_balance/run_model_based_corrective_teacher_case30_pair.sh"
)
SPEC = importlib.util.spec_from_file_location("corrective_pair_validator", VALIDATOR)
...
def _fixture_repo(tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
    ...
    paths = {
        ...
        "case30_plan": "artifacts/case30_plan.npz",
        "perturbation_profile": "artifacts/wrench.json",
    ...
    identities = {
        name: _identity(
            repo,
            relative,
            tracked=name in MODULE.TRACKED_IDENTITIES,
        )
        for name, relative in paths.items()
    }
    contract = {
...
def test_validator_rejects_forged_identity(tmp_path, monkeypatch) -> None:
    ...
    payload["identities"]["case30_plan"]["sha256"] = "0" * 64
