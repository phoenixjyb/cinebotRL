...
readonly WRENCH_PROFILE="$WIN_ROOT\\artifacts\\two_wheel_riser\\20260721_case30_perturbation_proposal_v1_cpu\\case30_wrench_profile.json"
readonly CORRECTIVE_PROFILE="$WIN_ROOT\\scripts\\two_wheel_balance\\model_based_corrective_teacher_case30_profile_v1.json"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  ...
  RISER_CORRECTIVE_CASE30_PERTURBATION RISER_CORRECTIVE_CASE30_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE30_AUTHORIZATION_FILE:-}"
...
timeout --signal=TERM --kill-after=30s 600 \
  ...
  --output "$OUTPUT_WIN\\baseline\\case_0030.json" \
...
if [[ "$BASELINE_STATUS" == 0 && "$BASELINE_GPU_RELEASE_PASSED" == 1 ]] \
  && python3 - "$OUTPUT/baseline/case_0030.json" <<'PY'
import json, sys
...
then
  ...
  timeout --signal=TERM --kill-after=30s 600 \
    ...
    --output "$OUTPUT_WIN\\candidate\\case_0030.json" \
