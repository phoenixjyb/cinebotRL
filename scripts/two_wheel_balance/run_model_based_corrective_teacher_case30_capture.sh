...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CORRECTIVE_CAPTURE_NAMESPACE RISER_CORRECTIVE_CAPTURE_CONTRACT \
  RISER_CORRECTIVE_CAPTURE_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CAPTURE_AUTHORIZATION_FILE:-}"
...
timeout --signal=TERM --kill-after=30s 600 \
  ...
  --output "$OUTPUT_WIN\\case_0030.json" \
  --headless \
