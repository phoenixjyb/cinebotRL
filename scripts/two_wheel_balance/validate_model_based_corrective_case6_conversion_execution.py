#!/usr/bin/env python3
"""Validate the one-shot case-6 CPU conversion execution contract."""
...
SCHEMA = "cinebotrl_two_wheel_riser_case6_conversion_execution_contract_v1"
ADMISSION_SCHEMA = (
    "cinebotrl_two_wheel_riser_case6_conversion_execution_admission_v1"
)
REVIEWED_PARENT = "18c000aa23316dfa98eeb25348c5cadd06d59f27"
EXPECTED_WSL_DISTRO = "Ubuntu"
CASE = 6
SPLIT = "train"
EXPECTED_SAMPLE_COUNT = 7933
NAMESPACE = "20260724_model_based_corrective_case6_conversion_v1_cpu"
...
EXPECTED_IDENTITY_PATHS = {
    ...
    "source_dynamic_gate": (
        "docs/03_training/two_wheel_balance/"
        "evidence_20260724_case6_corrective_capture_v1/case_0006.json"
    ),
    "source_admission": (
        "docs/03_training/two_wheel_balance/"
        "evidence_20260724_case6_corrective_capture_v1/admission.json"
    ),
    "source_capture_contract": (
        "docs/03_training/two_wheel_balance/"
        "evidence_20260724_case6_corrective_capture_v1/contract.json"
    ),
    "converter_cli": (
...
def _review_source(
    ...
    checks = {
        ...
        "clipping_audited": np.count_nonzero(clipped, axis=0).tolist()
        == [0, 146, 0],
        "three_clocks_preserved": all(
            len(payload[name]) == EXPECTED_SAMPLE_COUNT
            for name in ("elapsed_time_s", "execution_time_s", "source_time_s")
        ),
        "learning_closed": all(
    ...
    metrics = {
        ...
        "elapsed_clock_end_s": float(payload["elapsed_time_s"][-1]),
        "execution_clock_end_s": float(payload["execution_time_s"][-1]),
        "source_clock_end_s": float(payload["source_time_s"][-1]),
    }
...
def validate(
    ...
    *,
    ...
    try:
        source_checks, source_metrics = _review_source(
