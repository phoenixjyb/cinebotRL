...
def test_adaptive_dense_stride_smooths_micro_transitions_without_changing_duration(
    tmp_path: Path,
) -> None:
    reference_dir = tmp_path / "reference"
    reference_digest = write_reference_package(reference_dir)
    ...
    output_dir = tmp_path / "derived_dense_stride"
    manifest = derive_ep4_time_warp_package(
        reference_dir,
        raw_seed_dir,
        output_dir,
        config=TimeReparameterizationConfig(
            episode_index=1,
            waypoint_stride=4,
            dense_source_anchor_start_0based=2,
            dense_source_anchor_end_0based=8,
            dense_waypoint_stride=2,
            time_allocation_strategy="proportional_lower_bounds",
            diagnostic_transition_start_1based=2,
    ...
    assert manifest["metrics"]["waypoint_reduction"]["dense_waypoint_stride"] == 2
