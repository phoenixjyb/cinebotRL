#!/usr/bin/env python3
"""Observe case-16 effective residual projection without changing commands."""
...
def _load_engine():
    name = "_cinebotrl_case16_projection_telemetry_engine"
...
def inject_projection_telemetry(
    ...
    if (
        ...
        or results[0].get("case") != 16
    ):
        raise ValueError("case16 adapter requires exactly one case-16 result")
    results[0]["corrective_teacher_projection_telemetry"] = telemetry
    payload["case16_validation_projection_telemetry_adapter"] = {
        "schema": (
...
def main() -> int:
    ...
    if output.is_file():
        ...
        raise FileNotFoundError("case16 playback did not write its output")
    return exit_code
