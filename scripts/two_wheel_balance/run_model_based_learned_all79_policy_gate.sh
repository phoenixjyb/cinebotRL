...
RISER_POLICY_INSPECTOR_PYTHON="$ISAAC_PYTHON" python3 "$PREFLIGHT" \
  ...
  --source-manifest "$RISER_MODEL_BASED_LEARNED_ALL79_SOURCE_MANIFEST" \
  --lqr-gains "$RISER_MODEL_BASED_LEARNED_ALL79_LQR_GAINS" \
  --robot-build-audit "$RISER_MODEL_BASED_LEARNED_ALL79_ROBOT_BUILD_AUDIT" \
  --robot-usd "$RISER_MODEL_BASED_LEARNED_ALL79_ROBOT_USD" \
  --drive-profile-selection \
  "$RISER_MODEL_BASED_LEARNED_ALL79_DRIVE_PROFILE_SELECTION" \
  --validation-gate-report \
  "$RISER_MODEL_BASED_LEARNED_ALL79_VALIDATION_REPORT" \
  --holdout-gate-report "$RISER_MODEL_BASED_LEARNED_ALL79_HOLDOUT_REPORT" \
  --require-authorized \
  --output "$receipt"
