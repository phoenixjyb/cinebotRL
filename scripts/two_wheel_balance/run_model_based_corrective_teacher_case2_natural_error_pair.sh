...
readonly CONTRACT="$SCRIPT_DIR/model_based_corrective_teacher_case2_natural_error_pair_contract_v1.json"
readonly VALIDATOR="$SCRIPT_DIR/validate_model_based_corrective_teacher_case2_natural_error_pair.py"
readonly NAMESPACE="20260724_model_based_corrective_teacher_case2_natural_error_pair_v1_exclusive"
...
readonly PLAN_DIR="$WIN_ROOT\\docs\\03_training\\two_wheel_balance\\evidence_20260724_case2_pair_readiness_cpu_v1\\source"
readonly CORRECTIVE_PROFILE="$WIN_ROOT\\scripts\\two_wheel_balance\\model_based_corrective_teacher_case2_natural_error_profile_v1.json"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CORRECTIVE_CASE2_NAMESPACE RISER_CORRECTIVE_CASE2_CONTRACT \
  RISER_CORRECTIVE_CASE2_PROFILE RISER_CORRECTIVE_CASE2_PLAN \
  RISER_CORRECTIVE_CASE2_PERTURBATION RISER_CORRECTIVE_CASE2_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE2_AUTHORIZATION_FILE:-}"
...
timeout --signal=TERM --kill-after=30s 600 \
  ...
  --output "$OUTPUT_WIN\\baseline\\case_0002.json" \
...
if [[ "$BASELINE_STATUS" == 0 && "$BASELINE_GPU_RELEASE_PASSED" == 1 ]] \
  && python3 - "$OUTPUT/baseline/case_0002.json" <<'PY'
import json
import sys
...
then
  ...
  timeout --signal=TERM --kill-after=30s 600 \
    ...
    --output "$OUTPUT_WIN\\candidate\\case_0002.json" \
