...
readonly CONTRACT="$SCRIPT_DIR/model_based_corrective_teacher_case32_validation_natural_error_pair_contract_v1.json"
readonly VALIDATOR="$SCRIPT_DIR/validate_model_based_corrective_teacher_case32_validation_natural_error_pair.py"
readonly NAMESPACE="20260728_model_based_corrective_teacher_case32_validation_natural_error_pair_v1_coexistence"
...
readonly PLAYBACK_ADAPTER="$WIN_ROOT\\scripts\\two_wheel_balance\\smoke_riser_case32_validation_natural_error_pair.py"
readonly FINALIZER="$WIN_ROOT\\scripts\\two_wheel_balance\\summarize_model_based_corrective_teacher_case32_validation_natural_error_pair.py"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CORRECTIVE_CASE32_VALIDATION_NAMESPACE \
  RISER_CORRECTIVE_CASE32_VALIDATION_CONTRACT \
  RISER_CORRECTIVE_CASE32_VALIDATION_PROFILE \
  RISER_CORRECTIVE_CASE32_VALIDATION_PLAN \
  RISER_CORRECTIVE_CASE32_VALIDATION_PERTURBATION \
  RISER_CORRECTIVE_CASE32_VALIDATION_OUTPUT; do
...
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE32_VALIDATION_AUTHORIZATION_FILE:-}"
AUTHORIZATION_SHA256="${RISER_CORRECTIVE_CASE32_VALIDATION_AUTHORIZATION_SHA256:-}"
...
setsid timeout --signal=TERM --kill-after=30s 600 \
  ...
  --output "$OUTPUT_WIN\\baseline\\case_0032.json" \
...
if [[ "$BASELINE_STATUS" == 0 && "$BASELINE_GPU_RELEASE_PASSED" == 1 ]] \
  && python3 - "$OUTPUT/baseline/case_0032.json" <<'PY'
import json
import sys
...
then
  ...
  setsid timeout --signal=TERM --kill-after=30s 600 \
    ...
    --output "$OUTPUT_WIN\\candidate\\case_0032.json" \
