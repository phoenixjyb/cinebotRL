...
SCRIPT = (
    ...
    / "scripts/two_wheel_balance/summarize_model_based_corrective_teacher_case6_pair.py"
)
SPEC = importlib.util.spec_from_file_location("case6_pair_summary", SCRIPT)
...
def _gate(
    *,
    candidate: bool,
    position_p95: float = 0.110,
    source_duration_s: float = 15.942736,
) -> dict[str, object]:
    perturbation = {
        "enabled": True,
        "profile": {
    ...
    result = {
        ...
        "execution_duration_s": 17.737274605869224,
        "dynamic_quality_passed": True,
        "position_error_p95_m": position_p95 if candidate else 0.118125,
        "position_error_max_m": 0.125 if candidate else 0.127080,
        "attitude_error_max_deg": 0.45,
        "pitch_max_deg": 6.3,
        "riser_servo_error_max_m": 0.012,
        "action_saturation_ratio": 0.0,
        "residual_action_abs_max": (
            [0.50, 0.10, 0.05] if candidate else [0.0, 0.0, 0.0]
        ),
        "corrective_teacher_telemetry": {
            "normalized_action_abs_max": [0.50, 0.10, 0.05]
        },
...
def _fixture(tmp_path: Path, *, candidate_p95: float = 0.110):
    ...
    contract = {
        ...
        "case": 6,
        "split": "train",
        "controller_arguments": {"reset_seed": 20260722},
        "identities": {
            "case6_plan": {"sha256": "a" * 64},
...
def test_summary_rejects_weak_improvement(tmp_path) -> None:
    root, admission = _fixture(tmp_path, candidate_p95=0.117)
    result = _summarize(root, admission)
...
def test_summary_rejects_mismatched_source_clock(tmp_path) -> None:
    root, admission = _fixture(tmp_path)
    path = root / "candidate/case_0006.json"
