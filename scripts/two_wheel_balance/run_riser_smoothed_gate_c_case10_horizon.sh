...
AUTHORIZATION="AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE10_HORIZON300_V1"
CASES="10"
STAMP="20260718_gate_c_smoothed_case10_horizon300_wzkp105_v1_exclusive"
...
if [[ "${RISER_SMOOTHED_GATE_C_CASE10_HORIZON_AUTHORIZATION:-}" != "$AUTHORIZATION" ]]; then
  printf 'case10 horizon Gate C authorization is absent or unknown\n' >&2
  exit 7
fi
...
RUNNER="$ROOT/scripts/two_wheel_balance/run_riser_smoothed_gate_c_case10_horizon.sh"
...
assert_gpu_free() {
  ...
  if [[ -n "$playback_owners" || -n "$compute_owners" ]]; then
    printf 'case10 horizon GPU is not free\n' >&2
...
assert_no_competing_cpu() {
  ...
  if [[ -n "$competing_cpu" ]]; then
    printf 'case10 horizon CPU/disk ownership is not exclusive\n%s\n' "$competing_cpu" >&2
    return 1
  fi
}
...
wait_for_gpu_release() {
  ...
  printf 'case10 horizon GPU did not release within 90 seconds\n' >&2
  assert_gpu_free
}
...
"$PY" -X utf8 - \
  "$PORTFOLIO_WIN\\case_0010_smoothed_riser_plan_v1.npz" <<'PY'
import json
import sys
...
for path, case in zip(sys.argv[1:], (10,), strict=True):
    ...
    if not all(checks.values()):
        raise SystemExit(f"invalid case10 horizon case {case}: {checks}")
PY
...
for CASE in 10; do
  ...
  if [[ ! -s "$OUTPUT_WSL/gates/case_$(printf '%04d' "$CASE").json" ]] \
    ...
    python3 "$SUMMARIZER" --root "$OUTPUT_WSL" --git-commit "$COMMIT" \
      --cases "$CASES" --output "$OUTPUT_WSL/summary.json" >/dev/null
    printf 'case10 horizon Gate C stopped on case %s\n' "$CASE" >&2
    exit 4
  fi
...
ok = (
    summary.get("requested_cases") == [10]
    and summary.get("dynamically_passed_cases") == [10]
...
raise SystemExit(0 if ok else 6)
PY
printf 'case10 horizon Gate C closed: %s\n' "$OUTPUT_WSL"
