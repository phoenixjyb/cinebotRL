#!/usr/bin/env python3
"""Audit validation case 8 before designing a case-specific paired canary."""
...
SCHEMA = "cinebotrl_two_wheel_riser_case8_validation_pair_readiness_cpu_v1"
SELECTION_SCHEMA = (
...
ELIGIBLE_VALIDATION_CASES = [8, 16, 22, 32]
LIMITS = {
...
def _case8_selection_row(selection: Mapping[str, object]) -> dict[str, object]:
    ...
    if len(matches) != 1:
        raise ValueError("pair selection must contain exactly one case-8 row")
    return matches[0]
...
def _load_plan(
    ...
    if not isinstance(metadata, dict):
        raise ValueError("case-8 plan metadata must be an object")
...
def _single_result(gate: Mapping[str, object]) -> dict[str, object]:
    ...
    if not isinstance(results, list) or len(results) != 1:
        raise ValueError("case-8 gate must contain exactly one result")
    result = results[0]
    if not isinstance(result, dict):
        raise ValueError("case-8 gate result must be an object")
    return result
...
def audit_readiness(
    ...
    selection_row = _case8_selection_row(selection)
    ...
    selection_checks = {
        ...
        "all_selection_checks": isinstance(selection.get("checks"), Mapping)
        and bool(selection["checks"])
        and all(value is True for value in selection["checks"].values()),
        "selected_cases": selection.get("selected_cases") == SELECTED_CASES,
        "source_validation_cases": selection.get("source_validation_cases")
        == SOURCE_VALIDATION_CASES,
        "eligible_validation_cases": selection.get(
            "eligible_validation_cases"
        )
        == ELIGIBLE_VALIDATION_CASES,
        "minimum_validation_corpus": selection.get(
            "minimum_validation_corpus_cases"
        )
        == 2
        and selection.get("same_seed_pair_required_before_capture") is True,
        "train_profile_reuse_forbidden": selection.get(
            "case30_profile_reuse_authorized"
        )
        is False,
        "case8_role": selection_row.get("selection_role")
        == "same_seed_validation_paired_canary_required",
        "case8_checks": isinstance(selection_row.get("checks"), Mapping)
        ...
        "learning_closed": selection.get("dataset_conversion_authorized")
        is False
    ...
    if not all(selection_checks.values()):
        raise ValueError(
            f"case-8 pair selection checks failed: {selection_checks}"
        )
    ...
    source_time_s = np.asarray(arrays["source_time_s"], dtype=np.float64)
    source_anchor_execution_index = np.asarray(
        arrays["source_anchor_execution_index"], dtype=np.int64
    )
    initialization_time_s = np.asarray(
        arrays["initialization_time_s"], dtype=np.float64
    )
    initialization_state = np.asarray(
        arrays["initialization_state"], dtype=np.float64
    )
    ...
    source_execution_clock_checks = {
        ...
        "source_clock_strictly_increasing": bool(
            source_time_s.shape == (count,)
            and abs(float(source_time_s[0])) <= 1e-9
        ...
        "source_duration_metadata": bool(
            ...
            and np.isclose(
                ...
                float(plan["source_provenance"]["source_duration_s"]),
                rtol=0.0,
                atol=1e-9,
            )
        ),
        "source_anchor_map_exact": bool(
            source_anchor_execution_index.shape == (count,)
            and np.array_equal(
                source_anchor_execution_index,
    ...
    plan_checks = {
        ...
        and source_anchor_execution_index.shape == (count,)
        and initialization_time_s.shape == (0,)
        and initialization_state.shape == (0, 7)
        ...
        "execution_clock": bool(
            abs(float(time_s[0])) <= 1e-9
            and np.all(np.diff(time_s) > 0.0)
            and np.all(np.diff(execution_time_s) > 0.0)
            and np.array_equal(time_s, execution_time_s)
        ),
        "source_clock": bool(
            abs(float(source_time_s[0])) <= 1e-9
            and np.all(np.diff(source_time_s) > 0.0)
            and np.isclose(
                float(source_time_s[-1]),
                float(plan["source_provenance"]["source_duration_s"]),
        ...
        "source_execution_contract": all(
            source_execution_clock_checks.values()
        ),
    }
    if not all(plan_checks.values()):
        raise ValueError(f"case-8 plan checks failed: {plan_checks}")
    ...
    if not all(gate_checks.values()):
        raise ValueError(f"case-8 dynamic gate checks failed: {gate_checks}")
    ...
    if not all(metric_checks.values()):
        raise ValueError(f"case-8 metric checks failed: {metric_checks}")
    ...
    return {
        ...
        "case7_profile_reuse_authorized": False,
        "pair_profile_cpu_ready": False,
        "safe_window_absent_requires_structural_profile": not bool(windows),
        "next_bounded_action": (
            "design_case8_validation_corrective_and_perturbation_profiles_cpu_only"
        ),
