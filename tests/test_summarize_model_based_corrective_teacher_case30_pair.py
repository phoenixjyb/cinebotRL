...
SCRIPT = (
    ...
    / "scripts/two_wheel_balance/summarize_model_based_corrective_teacher_case30_pair.py"
)
SPEC = importlib.util.spec_from_file_location("corrective_pair_summary", SCRIPT)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


def _gate(*, candidate: bool, position_p95: float = 0.136) -> dict[str, object]:
    perturbation = {
        "enabled": True,
        "profile": {"case": 30, "force_body_x_n": 20.0, "duration_steps": 20},
        "trigger_step": 100,
    ...
    result = {
        ...
        "position_error_p95_m": position_p95 if candidate else 0.142,
        "position_error_max_m": 0.168 if candidate else 0.170,
...
def _fixture(tmp_path: Path, *, candidate_p95: float = 0.136):
    ...
    contract = {
        ...
        "case": 30,
        "split": "train",
        "controller_arguments": {"reset_seed": 20260746},
...
def test_summary_rejects_weak_improvement(tmp_path) -> None:
    root, admission = _fixture(tmp_path, candidate_p95=0.141)
    result = MODULE.summarize(
        root,
        admission,
        runtime_commit="b" * 40,
...
def test_summary_rejects_any_capture_path(tmp_path) -> None:
    root, admission = _fixture(tmp_path)
    path = root / "candidate/case_0030.json"
