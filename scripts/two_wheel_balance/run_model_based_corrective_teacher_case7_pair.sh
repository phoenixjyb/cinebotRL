...
readonly CONTRACT="$SCRIPT_DIR/model_based_corrective_teacher_case7_pair_contract_v1.json"
readonly VALIDATOR="$SCRIPT_DIR/validate_model_based_corrective_teacher_case7_pair.py"
readonly NAMESPACE="20260724_model_based_corrective_teacher_case7_pair_v1_exclusive"
...
readonly PLAN_DIR="$WIN_ROOT\\docs\\03_training\\two_wheel_balance\\evidence_20260724_case7_pair_readiness_cpu_v1\\source"
readonly WRENCH_PROFILE="$WIN_ROOT\\scripts\\two_wheel_balance\\model_based_corrective_teacher_case7_wrench_profile_v1.json"
readonly CORRECTIVE_PROFILE="$WIN_ROOT\\scripts\\two_wheel_balance\\model_based_corrective_teacher_case7_profile_v1.json"
...
readonly FINALIZER="$WIN_ROOT\\scripts\\two_wheel_balance\\summarize_model_based_corrective_teacher_case7_pair.py"
# This one-use authorization opens only the paired canary namespace.
readonly AUTHORIZATION_SHA256="c9a19cfaa1bb5f518bde8c2223ef0cfe6bfc87099e5daa7f7acc90f37b7bae9d"
...
for variable in RISER_ROOT RISER_WIN_ROOT ISAAC_PYTHON \
  RISER_CORRECTIVE_CASE7_NAMESPACE RISER_CORRECTIVE_CASE7_CONTRACT \
  RISER_CORRECTIVE_CASE7_PROFILE RISER_CORRECTIVE_CASE7_PLAN \
  RISER_CORRECTIVE_CASE7_PERTURBATION RISER_CORRECTIVE_CASE7_OUTPUT; do
  [[ -z "${!variable+x}" ]] || reject "conflicting_environment_override:$variable"
done

MODE="${1:---preflight}"
[[ "$MODE" == --preflight || "$MODE" == --execute ]] || reject "unsupported_mode" 2
AUTHORIZATION_FILE="${RISER_CORRECTIVE_CASE7_AUTHORIZATION_FILE:-}"
...
timeout --signal=TERM --kill-after=30s 600 \
  ...
  --output "$OUTPUT_WIN\\baseline\\case_0007.json" \
...
if [[ "$BASELINE_STATUS" == 0 && "$BASELINE_GPU_RELEASE_PASSED" == 1 ]] \
  && python3 - "$OUTPUT/baseline/case_0007.json" <<'PY'
import json
import sys
...
then
  ...
  timeout --signal=TERM --kill-after=30s 600 \
    ...
    --output "$OUTPUT_WIN\\candidate\\case_0007.json" \
