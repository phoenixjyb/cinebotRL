...
def test_smoothed_case74_gate_c_is_isolated_hash_bound_and_label_free() -> None:
    ...
    assert "AUTHORIZED_RISER_SMOOTHED_GATE_C_CASE74_RELIEF_WZKP090_V4" in source
    ...
    assert "20260718_gate_c_smoothed_case74_relief_wzkp090_v4_exclusive" in source
    ...
    assert 'CONTROLLER_WZ_KP="0.90"' in source
    assert '--controller-wz-kp "$CONTROLLER_WZ_KP"' in source
    assert 'gate.get("controller_overrides") == {"wz_kp": 0.9}' in source
