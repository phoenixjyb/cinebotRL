#!/usr/bin/env python3
"""Route Stage A rollout actions between fixed-base and base-required policies.

This evaluator mirrors evaluate_stage_rollout_gate.py but loads two PPO
checkpoints. Per environment, trajectories whose metadata filename contains
``base_required`` use the base-required checkpoint with scaled base rows; all
other trajectories use the default checkpoint with base rows frozen.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint_default", required=True)
    parser.add_argument("--checkpoint_base_required", required=True)
    ...
    parser.add_argument(
        "--base_required_action_scale",
        type=float,
        default=0.25,
        help="Scale base action rows [6,7,8] for routed base_required trajectories.",
    )
    parser.add_argument(
        "--default_freeze_base_actions",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Zero default-policy base rows [6,7,8] before env dynamics.",
    )
    parser.add_argument(
...
def base_required_mask(raw_env, num_envs: int) -> np.ndarray:
    ...
    if not metadata:
        return mask
    for env_id, item in enumerate(metadata[:num_envs]):
        if isinstance(item, dict) and "base_required" in str(item.get("file", "")):
            mask[env_id] = True
    return mask
...
def main() -> int:
    args = parse_args()
    if args.base_required_action_scale < 0.0 or args.base_required_action_scale > 1.0:
        raise ValueError("--base_required_action_scale must be in [0, 1]")
    checkpoint_default = Path(args.checkpoint_default)
    checkpoint_base_required = Path(args.checkpoint_base_required)
    vec_normalize = Path(args.vec_normalize)
    require(checkpoint_default.exists(), f"default checkpoint not found: {checkpoint_default}")
    require(checkpoint_base_required.exists(), f"base_required checkpoint not found: {checkpoint_base_required}")
    ...
    try:
        ...
        expected_obs_dim = int(np.prod(PPO.load(str(checkpoint_default), device="cpu").observation_space.shape))
        base_required_obs_dim = int(
            np.prod(PPO.load(str(checkpoint_base_required), device="cpu").observation_space.shape)
        )
        require(
            expected_obs_dim == base_required_obs_dim,
            f"checkpoint observation dims differ: default={expected_obs_dim}, "
            f"base_required={base_required_obs_dim}",
        )
        ...
        obs = env.reset()
        model_default = PPO.load(str(checkpoint_default), env=env, device="cuda:0")
        model_base_required = PPO.load(str(checkpoint_base_required), env=env, device="cuda:0")
        print(
            "[router] default checkpoint for non-base-required trajectories; "
            "base_required checkpoint for filenames containing 'base_required'",
            flush=True,
        )
        ...
        dones_count = 0
        route_base_required_samples = 0
        route_default_samples = 0
        ...
        for step in range(args.steps):
            if open_loop_actions is None:
                action_default, _ = model_default.predict(obs, deterministic=True)
                action_base_required, _ = model_base_required.predict(obs, deterministic=True)
                route_mask = base_required_mask(raw_env, args.num_envs)
                route_base_required_samples += int(np.count_nonzero(route_mask))
                route_default_samples += int(route_mask.size - np.count_nonzero(route_mask))
                if args.default_freeze_base_actions:
                    action_default = action_default.copy()
                    action_default[:, 6:9] = 0.0
                action_base_required = action_base_required.copy()
                action_base_required[:, 6:9] *= args.base_required_action_scale
                action = np.where(route_mask[:, None], action_base_required, action_default)
            else:
        ...
        metrics = {
            ...
            "checkpoint_default": str(checkpoint_default),
            "checkpoint_base_required": str(checkpoint_base_required),
            ...
            "default_freeze_base_actions": bool(args.default_freeze_base_actions),
            "base_required_action_scale": float(args.base_required_action_scale),
            "route_default_samples": int(route_default_samples),
            "route_base_required_samples": int(route_base_required_samples),
